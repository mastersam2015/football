...
//--------------------------------------onclick
	...
	$ready=$_GET["ready"];
	if(empty($ready)){
		echo "window.location.href='cos2.php?akx='+". $pakax ."+'&aky='+". $pakay ."+'&rx='+rx+'&ry='+ry;";
	}
...
kolej=data.split("a");
	// console.log(kolej[0]);	
	 if(kolej[0]==2){
	 window.location.href='cos2.php?akx='+kolej[1]+'&aky='+kolej[2]+"&resetuj=1";
	 }
});
	 }
...
$tab = json_decode($the_data);
	...
	$way=1;//echo "<script>alert('not way');</script>";
	} if($str[1] == "2" or $str[2] == "2" or $str[3] == "2" or $str[4] == "2"  or $str[6] == "2" or $str[7] == "2"){
	$line.="ctx.beginPath(); ctx.lineWidth=\"2\"; ctx.strokeStyle=\"black\"; ctx.moveTo(".$aky.",".$akx."); ctx.lineTo(".$rx.",".$ry."); ctx.stroke(); ";
			$dotab="a".$str[1]."a".$str[2]."a".$str[3]."a".$str[4]."a".$str[5]."a".$str[6]."a".$str[7]."a2"; $pom="1".$rx."".$ry; $tab->$pom=$dotab;
		$dotab="a".$str[1]."a".$str[2]."a".$str[3]."a2"."a".$str[5]."a".$str[6]."a".$str[7]."a".$str[8]; $pom="1".$aky."".$akx;$tab->$pom=$dotab;
	$ping=1;//echo "<script>alert('ping');</script>";
	...
	$notway=0;
	//header("Location: cos2.php?akx=".$ry."&aky=".$rx);
	if($ping!=1){
	if($turn==1){
		//echo "<script>alert('kolej przeciwnika');</script>";
		//echo "<script>window.location.href='cos2.php?ready=1';</script>";
		 $fp = fopen("ready.txt", "w");
...
echo "<script>window.location.href='cos2.php?ready=1';</script>";
	...
	if($way==1){
		echo "<script>window.location.href='cos2.php?akx=".$ry."&aky=".$rx."';</script>";
	}else{
	echo "<script>window.location.href='cos2.php?akx=".$ry."&aky=".$rx."';</script>";
	}
